...
function getOrComputeFullDivergence(marketData, indicator, config) {
    ...
    if (indicator === 'RSI') {
        ...
        let fullMACD = calculateFullMACD(marketData, config);
        if (!fullMACD) {
            // هم‌راستا با calculateRSI (که در همین حالت به‌جای throw یک آرایه‌ی خالی
            // برمی‌گرداند): وقتی طول داده کمتر از حداقل لازم (slowPeriod + signalPeriod)
            // است، هیچ مقدار معتبر MACD برای هیچ کندلی از نظر ریاضی وجود ندارد — این یک
            // خطای واقعی نیست، فقط یعنی «هنوز تاریخچه‌ی کافی نیست». قبلاً throw کردن اینجا
            // باعث می‌شد کل فایل/چانک (نه فقط این تایم‌فریم) fail شود، دقیقاً مثل باگی که
            // برای فایل‌های کوتاه resample‌شده به تایم‌فریم‌های بالا (مثل 24h) رخ می‌داد.
            console.error(`[DIVERGENCE] ❌ [MACD] داده‌ها کافی نیستند (طول: ${marketData.length}) — بدون سیگنال واگرایی MACD ادامه داده می‌شود.`);
            const nullArray = new Array(marketData.length).fill(null);
            fullMACD = { macdLine: nullArray, signalLine: nullArray, histogram: nullArray };
        }
...
function precomputeIndicator(marketData, indicator, config = DIVERGENCE_CONFIG) {
    if (indicator === 'RSI') {
        ...
        if (!fullMACD) {
            // همان دلیل بالا در getOrComputeFullDivergence: داده‌ی کوتاه‌تر از حداقل لازم
            // یک خطای واقعی نیست، فقط یعنی این بخش از دیتاست هنوز تاریخچه‌ی کافی برای MACD
            // ندارد؛ throw کردن اینجا کل پردازش فایل (نه فقط این تایم‌فریم) را متوقف می‌کرد.
            console.error(`[DIVERGENCE] ❌ [MACD] داده‌ها کافی نیستند (طول: ${marketData.length}) — بدون سیگنال واگرایی MACD ادامه داده می‌شود.`);
            const nullArray = new Array(marketData.length).fill(null);
            return { macdLine: nullArray, signalLine: nullArray, histogram: nullArray };
        }
