...
function loadInitialTfBufferMap(dataDir, startIndex, lookbackCount, bufferSize) {
    const initialMap = {};
    for (const tf of PRELOAD_TF_CONFIG) initialMap[tf.label] = [];

    if (startIndex <= 0) {
        log('startIndex برابر ۰ است، بافر اولیه لازم نیست (شروع دیتاست).');
        return initialMap;
    }

    const prevFileNames = getPreviousFileNames(dataDir, startIndex, lookbackCount);
    if (prevFileNames.length === 0) {
        log('⚠️ هیچ فایل قبلی برای بافر اولیه یافت نشد.', 'WARN');
        return initialMap;
    }

    log(`بارگذاری بافر اولیه از ${prevFileNames.length} فایل قبل از این چانک: ${prevFileNames.join(', ')}`);
    ...
    for (const tf of PRELOAD_TF_CONFIG) {
        log(`بافر اولیه تایم‌فریم ${tf.label}: ${initialMap[tf.label].length} کندل بارگذاری شد.`);
    }

    return initialMap;
}

const initialTfBufferMap = loadInitialTfBufferMap(dataDir, startIndex, PRELOAD_LOOKBACK_FILES, FIXED_BUFFER_CANDLES_PRELOAD);
...
(async () => {
    ...
    // بافر previousTfDataMap با initialTfBufferMap (که قبل از این IIFE و قبل از حلقه‌ی
    // فایل‌ها از ۷۵ فایل ماقبل این چانک محاسبه شد) مقداردهی اولیه می‌شود تا اولین فایل
    // این چانک هم مثل بقیه‌ی فایل‌ها بافر ۷۲۰ کندلی کامل داشته باشد.
    const previousTfDataMap = { ...initialTfBufferMap };
