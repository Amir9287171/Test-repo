...
(async () => {
    ...
    // فقط یک‌بار (مستقل از فایل/تایم‌فریم) از نسخه‌ی اصلی بارگذاری و رمزگشایی می‌شود.

    let inverseEntryTimesSet = null;

    if (isInverseStrategy) {

        const aggTradesPath = path.join(process.cwd(), 'aggregated', baseStrategyName, `${baseStrategyName}_trades.enc`);

        log(`📥 بارگذاری زمان‌های ورود نسخه‌ی اصلی از: ${aggTradesPath}`);

        if (!fs.existsSync(aggTradesPath)) {

            log(`❌ فایل یکپارچه‌ی معاملات نسخه‌ی اصلی یافت نشد: ${aggTradesPath}`, 'CRITICAL');
        ...
        try {
            ...
            inverseEntryTimesSet = extractEntryTimesFromTradesEnc(aggTradesPath, resultsPassword);
        ...
        log(`✅ ${inverseEntryTimesSet.size} زمان ورود منحصربه‌فرد از معاملات ${baseStrategyName} بارگذاری شد.`);
        ...
        try {
            ...
            for (const tf of timeframes) {
                ...
                const result = await backtestCore.runBacktest(tfData, {
                    ...
                    inverseEntryTimes: isInverseStrategy ? Array.from(inverseEntryTimesSet) : undefined,
