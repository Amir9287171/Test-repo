...
// اضافه شد: پارامتردهی به واگرایی از طریق CLI — دقیقاً برای همان نیازی که خطوط روند
// از طریق ANALYSIS_CONFIG.trendLines در فایل استراتژی دارند، اما چون هدف اینجا اسکن
// خودکارِ صدها ترکیب پارامتر روی چند نماد/ماه است، اجازه دادن به override از طریق
// آرگومان‌های خط فرمان (بدون نیاز به ساختن/ویرایش فایل استراتژی برای هر ترکیب) لازم بود.
// --divergence-config یک رشته JSON با هر کلیدی از DIVERGENCE_CONFIG (مثلاً PIVOT_PERIOD،
// RSI_PERIOD، LOOKBACK_PIVOTS، ...) می‌پذیرد. --divergence-config-file مسیر یک فایل JSON
// معادل است (برای config های طولانی که در CLI جا نمی‌شوند). اگر هیچ‌کدام داده نشوند،
// رفتار دقیقاً مثل قبل است (پیش‌فرض‌های DIVERGENCE_CONFIG داخل divergence-detector.js).
const divergenceConfigCliRaw = getArg('--divergence-config');
const divergenceConfigCliFile = getArg('--divergence-config-file');
const divergenceIndicatorsCli = getArg('--divergence-indicators'); // مثلاً "RSI" یا "RSI,MACD"

// اضافه شد: امکان اجرای فقط زیرمجموعه‌ای از تایم‌فریم‌ها (مثلاً فقط "4h,24h") — برای گام
// استثنایی بکتست‌گیری از تایم‌فریم‌های تازه‌اضافه‌شده روی استراتژی‌هایی که تایم‌فریم‌های
// قدیمی‌شان از قبل کامل شده. اگر داده نشود، رفتار قبلی (همه‌ی تایم‌فریم‌ها) حفظ می‌شود.
const timeframesOnlyCli = getArg('--timeframes-only');
const timeframesOnlyFilter = timeframesOnlyCli
    ? timeframesOnlyCli.split(',').map(s => s.trim()).filter(Boolean)
    : null;

let divergenceConfigCli = null;
if (divergenceConfigCliFile) {
    try {
        divergenceConfigCli = JSON.parse(fs.readFileSync(divergenceConfigCliFile, 'utf8'));
    } catch (err) {
        log(`❌ خواندن/پارس --divergence-config-file شکست خورد: ${err.message}`, 'ERROR');
        process.exit(1);
    }
} else if (divergenceConfigCliRaw) {
    try {
        divergenceConfigCli = JSON.parse(divergenceConfigCliRaw);
    } catch (err) {
        log(`❌ پارس JSON مقدار --divergence-config شکست خورد: ${err.message}`, 'ERROR');
        process.exit(1);
    }
}
...
log(`شروع run-backtest.js با آرگومان‌ها: strategyFile=${strategyFile}, startIndex=${startIndex}, chunkSize=${chunkSize}, dataDir=${dataDir}, maxContinuation=${maxContinuationFiles}, aggregatedRepoDir=${aggregatedRepoDir}, timeframesOnly=${timeframesOnlyFilter ? timeframesOnlyFilter.join(',') : 'همه'}`);
...
function getFileNamesByRange(dataDir, startIndex, count) {
    ...
    // ⚠️ رفع باگ: قبلاً این تابع مستقلاً fs.readdirSync + مرتب‌سازی فقط-تاریخ را
    // دوباره پیاده می‌کرد (کد تکراری با getPreviousFileNames/sortedDatedFiles
    // بالاتر). حالا همان تابع مشترک sortedDatedFiles با ترتیب قطعی (تاریخ، سپس
    // نام فایل) استفاده می‌شود تا «چانک N» همیشه دقیقاً همان ۱۰۰ فایل را نشان
    // بدهد — چه در این اجرای run-backtest.js، چه در getPreviousFileNames.
    const dated = sortedDatedFiles(dataDir);
...
// ====================== بافر اولیه (look-back) برای اولین فایل چانک ======================
// وقتی startIndex > 0 است یعنی این چانک ادامه‌ی چانک‌های قبلی‌ست و فایل‌های قبل از آن
// در دیتاست وجود دارند. previousTfDataMap فقط بین فایل‌های داخل همین چانک بافر ۷۲۰
// کندلی رد و بدل می‌کند، پس بدون این بخش، اولین فایل هر چانک (به‌جز چانک صفر) با
// بافر خالی شروع می‌شود و اندیکاتورهای دوره‌بلند (SMA200 و مشابه) در ابتدای آن فایل
// غلط محاسبه می‌شوند. اینجا ۷۵ فایل خام قبل از این چانک (بر اساس ترتیب تاریخ، همان
// ترتیبی که getFileNamesByRange استفاده می‌کند) بارگذاری و برای هر ۶ تایم‌فریم
// ریسمپل می‌شوند و آخرین ۷۲۰ کندل هر تایم‌فریم نگه داشته می‌شود. این کار فقط یک‌بار
// در ابتدای چانک انجام می‌شود، نه به ازای هر فایل.

const PRELOAD_LOOKBACK_FILES = 75;
const FIXED_BUFFER_CANDLES_PRELOAD = 720;
const PRELOAD_TF_CONFIG = [
    { label: '5m', minutes: 5 },
...
// ⚠️ رفع باگ (چند نماد در یک چانک): استخراج نماد از نام فایل، دقیقاً هم‌فرمت با
// symbolKey که پایین‌تر برای هر فایل در حلقه‌ی اصلی محاسبه می‌شود (مثلاً
// "BTCUSDT-1m" از "BTCUSDT-1m-2018-01-01_2018-01-10.csv").
// ⚠️ رفع باگ (سکوت در برابر خطای فایل‌به‌فایل): قبلاً وقتی یک فایل خالی بود یا
// پردازشش استثنا می‌داد، فقط یک لاگ WARN/ERROR در کنسول چاپ می‌شد و ادامه
// می‌داد — بدون این‌که results.enc آن فایل/تایم‌فریم هرگز نوشته شود. چون این
// جاب با موفقیت (exit code صفر) تمام می‌شود، استپ «۶. ذخیره لاگ خطا» در
// backtest.yml که فقط با if: failure() اجرا می‌شود هرگز فعال نمی‌شد؛ یعنی
// این خطا هیچ‌جا به‌جز لاگ خام همان جاب دیده نمی‌شد و آن فایل/چانک برای همیشه،
// در هر تلاش مجدد، دوباره «ناقص» می‌ماند بدون این‌که کسی متوجه دلیلش شود.
// این تابع یک مارکر ساده در errors/ می‌نویسد تا در همان مسیر معمولیِ
// جمع‌آوری/آپلود خطاها قابل مشاهده باشد.
function recordFileError(strategyNameArg, startIndexArg, fileName, tfLabel, reason) {
    try {
        const errorsDir = path.join(process.cwd(), 'errors');
        fs.mkdirSync(errorsDir, { recursive: true });
        const safeFile = fileName.replace(/[^a-zA-Z0-9._-]/g, '_');
        const safeTf = tfLabel ? `_${tfLabel}` : '';
        const marker = path.join(errorsDir, `SKIP_${strategyNameArg}_${startIndexArg}_${safeFile}${safeTf}.json`);
        fs.writeFileSync(marker, JSON.stringify({
            strategy: strategyNameArg,
            startIndex: startIndexArg,
            fileName,
            tfLabel: tfLabel || null,
            reason,
            at: new Date().toISOString()
        }, null, 2), 'utf8');
    } catch (e) {
        log(`⚠️ نوشتن مارکر خطا برای ${fileName} ممکن نشد: ${e.message}`, 'WARN');
    }
}

function symbolOfFile(fileName) {
    return path.basename(fileName, path.extname(fileName)).split('-').slice(0, 2).join('-');
}

// ⚠️ رفع باگ (ترتیب غیرقطعی): قبلاً مرتب‌سازی فقط بر اساس تاریخ بود
// (a.date.localeCompare(b.date))؛ برای فایل‌های هم‌تاریخ از نمادهای مختلف
// (که در این دیتاست همیشه پیش می‌آید، چون هر نماد برای همان بازه‌ی تاریخی یک
// فایل دارد)، ترتیب نهایی به ترتیب fs.readdirSync (که در سطح سیستم‌عامل
// تضمین‌شده نیست) وابسته می‌ماند. یعنی "چانک N" حتی بین دو اجرای متوالی روی
// همان چک‌اوت هم می‌توانست تئوریاً عوض شود. حالا با افزودن نام فایل به‌عنوان
// معیار دوم، ترتیب کاملاً قطعی و تکرارپذیر است.
function sortedDatedFiles(dataDir) {
    ...
    dated.sort((a, b) => a.date.localeCompare(b.date) || a.file.localeCompare(b.file));
    return dated;
}

function getPreviousFileNames(dataDir, startIndex, lookbackCount) {
    const dated = sortedDatedFiles(dataDir);
    const fromIdx = Math.max(0, startIndex - lookbackCount);
    return dated.slice(fromIdx, startIndex).map(x => x.file);
}

// ⚠️ رفع باگ (اختلاط بافر بین نمادها): بافر ۷۲۰-کندلی previousTfDataMap فقط
// بر اساس تایم‌فریم نگه‌داری می‌شد، نه نماد. چون فایل‌های چانک بر اساس تاریخ
// (نه نماد) مرتب‌اند، وقتی چانک شامل چند نماد باشد (مثلاً BTCUSDT سپس ETHUSDT
// با تاریخ یکسان)، کندل‌های انتهای BTCUSDT به اشتباه به‌عنوان «بافر قبلی»
// وارد محاسبه‌ی اندیکاتورهای ETHUSDT می‌شد. این نسخه، بافر اولیه (preload) را
// هم به‌ازای همان نماد و فقط از فایل‌های قبلیِ همان نماد می‌سازد.
function getPreviousFileNamesForSymbol(dataDir, startIndex, lookbackCount, symbolKey) {
    const dated = sortedDatedFiles(dataDir);
    const sameSymbolBeforeStart = dated
        .filter((x, idx) => idx < startIndex && symbolOfFile(x.file) === symbolKey)
        .map(x => x.file);
    return sameSymbolBeforeStart.slice(-lookbackCount);
}

// ⚠️ رفع باگ (اختلاط بافر بین نمادها، ادامه): این تابع قبلاً یک‌بار، برای کل
// چانک (بدون توجه به نماد)، از N فایلِ «قبلیِ» چانک (بر ترتیب کلیِ تاریخ، روی
// همه‌ی نمادها) بافر اولیه می‌ساخت. چون فایل‌های همان چانک هم به ترتیب تاریخ
// (نه نماد) پردازش می‌شوند، این یعنی بافر اولیه‌ی مثلاً ETHUSDT می‌توانست از
// کندل‌های BTCUSDT ساخته شده باشد. حالا این تابع per-symbol است: برای نمادِ
// داده‌شده، فقط از فایل‌های قبلیِ همان نماد (نه هر نمادی) بافر می‌سازد، و فقط
// در اولین باری که آن نماد در این چانک دیده می‌شود صدا زده می‌شود (نگاه کنید
// به getBufferMapForSymbol پایین‌تر).
function loadInitialTfBufferMapForSymbol(dataDir, startIndex, lookbackCount, bufferSize, symbolKey) {
    const initialMap = {};
    for (const tf of PRELOAD_TF_CONFIG) initialMap[tf.label] = [];

    if (startIndex <= 0) {
        log(`startIndex برابر ۰ است، بافر اولیه برای نماد ${symbolKey} لازم نیست (شروع دیتاست).`);
        return initialMap;
    }

    const prevFileNames = getPreviousFileNamesForSymbol(dataDir, startIndex, lookbackCount, symbolKey);
    if (prevFileNames.length === 0) {
        log(`⚠️ هیچ فایل قبلیِ نماد ${symbolKey} برای بافر اولیه یافت نشد.`, 'WARN');
        return initialMap;
    }

    log(`بارگذاری بافر اولیه‌ی نماد ${symbolKey} از ${prevFileNames.length} فایل قبل از این چانک: ${prevFileNames.join(', ')}`);

    for (const prevFileName of prevFileNames) {
        let prevMarketData;
        try {
            prevMarketData = loadSingleFile(dataDir, prevFileName);
        } catch (e) {
            log(`⚠️ خطا در بارگذاری فایل قبلی ${prevFileName} برای بافر اولیه: ${e.message}`, 'WARN');
            continue;
        }
        if (!prevMarketData || prevMarketData.length === 0) continue;

        for (const tf of PRELOAD_TF_CONFIG) {
            const tfData = resampleOHLC(prevMarketData, tf.minutes);
            initialMap[tf.label] = [...initialMap[tf.label], ...tfData].slice(-bufferSize);
        }
    }

    for (const tf of PRELOAD_TF_CONFIG) {
        log(`بافر اولیه‌ی نماد ${symbolKey}، تایم‌فریم ${tf.label}: ${initialMap[tf.label].length} کندل بارگذاری شد.`);
    }

    return initialMap;
}

// previousTfDataMapBySymbol: { [symbolKey]: { [tfLabel]: candle[] } }
// جایگزین initialTfBufferMap/previousTfDataMap قبلی که فقط بر اساس تایم‌فریم
// بود. هر نماد بافر مستقل خودش را دارد؛ بافر هر نماد فقط در اولین برخورد با
// آن نماد در همین چانک (lazy) ساخته می‌شود.
const previousTfDataMapBySymbol = {};

function getBufferMapForSymbol(symbolKey) {
    if (!previousTfDataMapBySymbol[symbolKey]) {
        previousTfDataMapBySymbol[symbolKey] = loadInitialTfBufferMapForSymbol(
            dataDir, startIndex, PRELOAD_LOOKBACK_FILES, FIXED_BUFFER_CANDLES_PRELOAD, symbolKey
        );
    }
    return previousTfDataMapBySymbol[symbolKey];
}

// ====================== manifest.json چانک ======================
// این فایل تنها منبع معتبر برای تعیین تعداد خروجی‌های مورد انتظار این چانک است
// (fileNames.length × تعداد تایم‌فریم‌ها). استپ ۷ در backtest.yml قبل از commit کردن
// progress.json این manifest را می‌خواند و برای هر ترکیب fileName × tfLabel وجود
// results/${strategyName}_${tfLabel}/${fileName}/results.enc را روی origin/main
// بررسی می‌کند.
function writeChunkManifest(strategyName, startIndex, fileNamesList, tfLabelsList) {
    const resultsDir = path.join(process.cwd(), 'results');
    fs.mkdirSync(resultsDir, { recursive: true });
    const manifestPath = path.join(resultsDir, `.manifest_${strategyName}_${startIndex}.json`);
    const manifest = { fileNames: fileNamesList, tfLabels: tfLabelsList };
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
    log(`manifest نوشته شد: ${manifestPath} (${fileNamesList.length} فایل × ${tfLabelsList.length} تایم‌فریم = ${fileNamesList.length * tfLabelsList.length} خروجی مورد انتظار)`);
}

// اگر --timeframes-only داده شده باشد، فقط همان تایم‌فریم‌ها واقعاً پردازش و روی دیسک
// نوشته می‌شوند (نگاه کنید به فیلتر «timeframes» پایین‌تر). اگر manifest بدون این فیلتر
// همیشه هر ۶ تایم‌فریم PRELOAD_TF_CONFIG را لیست کند، استپ ۷ در backtest.yml خروجی‌های
// فیلتر‌شده (که اصلاً قرار نبوده تولید شوند) را «غایب» تشخیص می‌دهد و به‌اشتباه کل چانک
// را fail می‌کند. برای سازگاری با استپ راستی‌آزمایی، manifest باید همان زیرمجموعه‌ای را
// منعکس کند که واقعاً روی دیسک نوشته خواهد شد.
const manifestTfLabels = timeframesOnlyFilter
    ? PRELOAD_TF_CONFIG.map(tf => tf.label).filter(label => timeframesOnlyFilter.includes(label))
    : PRELOAD_TF_CONFIG.map(tf => tf.label);
writeChunkManifest(strategyName, startIndex, fileNames, manifestTfLabels);
...
// JSON باشد، یا مستقیماً یک JSON خام)، آرایه‌ی کامل تمام رکوردهای ترید (شامل فیلد symbol

// که در مرحله‌ی aggregate-results روی هر ترید نوشته می‌شود) را استخراج می‌کند. فیلتر نماد

// این‌جا انجام نمی‌شود؛ چون فایل per-تایم‌فریم شامل معاملات همه‌ی نمادهاست و فیلتر باید

// بعداً و به‌ازای هر نماد جداگانه روی همین آرایه‌ی خام انجام شود (نه این‌که entryTimeها

// بدون توجه به symbol در یک Set مشترک قاطی شوند).

function extractAllTradesFromTradesEnc(encPath, password) {
...
(async () => {
    ...
    // آرایه‌ی خام تریدها (شامل symbol) فقط یک‌بار به‌ازای هر تایم‌فریم بارگذاری/رمزگشایی
    // می‌شود؛ Set زمان‌های ورود اما به‌ازای هر ترکیب (تایم‌فریم + نماد) جداگانه ساخته و
    // کش می‌شود، چون فایل یکپارچه شامل معاملات همه‌ی نمادها با هم است.
    const rawTradesByTf = {};
    const inverseEntryTimesByTfSymbol = {};

    function getRawTradesForTf(tfLabel) {

        if (rawTradesByTf[tfLabel]) return rawTradesByTf[tfLabel];
        ...
        log(`📥 بارگذاری معاملات نسخه‌ی اصلی (تایم‌فریم ${tfLabel}) از: ${aggTradesPath}`);
        ...
        try {
            ...
            trades = extractAllTradesFromTradesEnc(aggTradesPath, resultsPassword);
        ...
        if (!trades || trades.length === 0) {
            ...
            log(`❌ هیچ تریدی از ${aggTradesPath} استخراج نشد.`, 'CRITICAL');

            process.exit(1);

        }

        const missingSymbolCount = trades.filter(t => t.symbol === undefined).length;

        if (missingSymbolCount > 0) {

            log(`⚠️ ${missingSymbolCount} ترید بدون فیلد symbol در ${aggDirName} یافت شد (احتمالاً داده‌ی قدیمی).`, 'WARN');

        }

        log(`✅ ${trades.length} ترید (همه‌ی نمادها) برای تایم‌فریم ${tfLabel} از ${aggDirName} بارگذاری شد.`);

        rawTradesByTf[tfLabel] = trades;

        return trades;

    }

    function getInverseEntryTimesForTf(tfLabel, symbol) {

        const cacheKey = `${tfLabel}__${symbol}`;

        if (inverseEntryTimesByTfSymbol[cacheKey]) return inverseEntryTimesByTfSymbol[cacheKey];

        const allTrades = getRawTradesForTf(tfLabel);

        const filtered = allTrades.filter(t => t.symbol === symbol);
        ...
        for (const t of filtered) {
            ...
            if (!isNaN(time)) set.add(time);

        }

        if (set.size === 0) {

            const availableSymbols = [...new Set(allTrades.map(t => t.symbol))].join(', ');

            log(`❌ هیچ entryTime معتبری برای نماد "${symbol}" در تایم‌فریم ${tfLabel} یافت نشد. نمادهای موجود در فایل: ${availableSymbols}`, 'CRITICAL');

            process.exit(1);

        }

        log(`✅ ${set.size} زمان ورود منحصربه‌فرد برای نماد "${symbol}" (تایم‌فریم ${tfLabel}) بارگذاری شد.`);

        inverseEntryTimesByTfSymbol[cacheKey] = set;
    ...
    // اضافه شد: تایم‌فریم‌های 4h (240 دقیقه) و 24h (1440 دقیقه)
    const tfLabels = ['5m', '15m', '30m', '1h', '4h', '24h'];
    ...
    // نگهداری بافر ثابت (rolling) به ازای هر تایم‌فریم: به‌جای نگه‌داشتن چند فایل کامل قبلی،

    // اینجا فقط یک آرایه‌ی صاف از حداکثر FIXED_BUFFER_CANDLES (۷۲۰) کندل آخرِ همان تایم‌فریم

    // نگه‌داری می‌شود. این روش مستقل از تعداد فایل‌های قبلی و اندازه‌ی هر فایل کار می‌کند —

    // برای تایم‌فریم‌های ریز (مثلاً 5m) معمولاً از همان یک فایل قبلی پر می‌شود و برای

    // تایم‌فریم‌های خیلی درشت (مثلاً 24h) به‌مرور از چندین فایل قبلی جمع می‌شود.

    const FIXED_BUFFER_CANDLES = 720;

    // ⚠️ رفع باگ (اختلاط بافر بین نمادها): قبلاً اینجا یک previousTfDataMap
    // واحد برای کل چانک ساخته می‌شد (initialTfBufferMap ثابت، محاسبه‌شده قبل
    // از این حلقه). چون فایل‌های چانک به ترتیب تاریخ (نه نماد) هستند، آن یک
    // بافر مشترک بین نمادهای مختلف رد و بدل می‌شد. حالا previousTfDataMap
    // داخل حلقه، به‌ازای هر فایل، از getBufferMapForSymbol(symbolKey) گرفته
    // می‌شود — یعنی هر نماد بافر ۷۲۰-کندلی مستقل خودش را دارد.
    ...
    for (let fileIdx = 0; fileIdx < fileNames.length; fileIdx++) {
        ...
        // نماد این فایل، دقیقاً هم‌فرمت با COIN_NAME که در loop_analysis.yml ساخته می‌شود:
        // COIN_NAME=$(basename "$FOLDER" | cut -d'-' -f1-2)  →  مثلاً "PAXGUSDT-1m"
        // اگر basename فایل خام با basename پوشه‌ی results فرق دارد، این خط باید مطابق‌سازی شود.
        const symbolKey = path.basename(fileName, path.extname(fileName)).split('-').slice(0, 2).join('-');

        // بافر ۷۲۰-کندلیِ مخصوص همین نماد (نه کل چانک) — نگاه کنید به توضیح بالا.
        const previousTfDataMap = getBufferMapForSymbol(symbolKey);
        ...
        try {
            ...
            if (marketData.length === 0) {
                ...
                recordFileError(strategyName, startIndex, fileName, null, 'empty_market_data');
            ...
            // اصلاح: قبلاً همیشه { indicators: ['RSI','MACD'] } ثابت بود، بدون هیچ راهی
            // برای تنظیم اندیکاتورها یا پارامترهای داخلی واگرایی — برخلاف trendLines/
            // ichimoku که از ANALYSIS_CONFIG استراتژی خوانده می‌شوند. الان دقیقاً همان
            // الگو برای واگرایی هم پیاده شده: ANALYSIS_CONFIG.divergence در فایل استراتژی
            // (اختیاری) پایه است، و --divergence-config/--divergence-indicators در CLI
            // (اگر داده شوند) رویش override می‌کنند — برای اسکن پارامتری روی چند فایل/نماد
            // بدون دست‌زدن به فایل استراتژی.
            const strategyDivergenceSettings = (analysisConfig && analysisConfig.divergence && typeof analysisConfig.divergence === 'object')
                ? analysisConfig.divergence
                : {};
            const divergenceIndicatorsFinal = divergenceIndicatorsCli
                ? divergenceIndicatorsCli.split(',').map(s => s.trim()).filter(Boolean)
                : (Array.isArray(strategyDivergenceSettings.indicators) ? strategyDivergenceSettings.indicators : ['RSI', 'MACD']);
            const divergenceConfigFinal = Object.assign(
                {},
                (strategyDivergenceSettings.config && typeof strategyDivergenceSettings.config === 'object') ? strategyDivergenceSettings.config : {},
                divergenceConfigCli || {}
            );
            const divergenceSettings = (needsDivergence && divergenceDetector)
                ? {
                    indicators: divergenceIndicatorsFinal,
                    recomputeInterval: strategyDivergenceSettings.recomputeInterval,
                    config: Object.keys(divergenceConfigFinal).length > 0 ? divergenceConfigFinal : undefined
                }
                : null;
            if (divergenceSettings) {
                log(`تنظیمات واگرایی نهایی: ${JSON.stringify(divergenceSettings)}`);
            }
            ...
            // اضافه شد: 4h (240 دقیقه) و 24h (1440 دقیقه)
            const allTimeframes = [
            ...
            const timeframes = timeframesOnlyFilter
                ? allTimeframes.filter(tf => timeframesOnlyFilter.includes(tf.label))
                : allTimeframes;
            ...
            for (const tf of timeframes) {
                ...
                // بافر ثابت (rolling) این تایم‌فریم: آرایه‌ی صافِ حداکثر ۷۲۰ کندل آخر، مستقل از
                ...
                // کمکی برای به‌روزرسانی بافر ثابت بعد از پردازش این فایل (چه از قبل انجام شده

                // باشد چه الان اجرا شود): آخرین FIXED_BUFFER_CANDLES کندل را نگه می‌داریم.

                const updateRollingBuffer = () => {

                    previousTfDataMap[tfLabel] = [...previousTfDataListForBuffer, ...tfData].slice(-FIXED_BUFFER_CANDLES);
                ...
                if (enableSmartContinuation && previousTfDataListForBuffer.length > 0) {
                    ...
                    log(`🧩 بافر ثابت (${previousTfDataListForBuffer.length} از حداکثر ${FIXED_BUFFER_CANDLES} کندل، تایم‌فریم ${tfLabel}) به ابتدای این فایل الحاق خواهد شد.`);
                ...
                const result = await backtestCore.runBacktest(tfData, {
                    ...
                    inverseEntryTimes: isInverseStrategy ? Array.from(getInverseEntryTimesForTf(tfLabel, symbolKey)) : undefined,
            ...
            recordFileError(strategyName, startIndex, fileName, null, `exception: ${err.message}`);
