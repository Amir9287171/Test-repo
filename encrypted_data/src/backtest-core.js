...
async function runBacktest(marketData, options, onProgress) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const LOG_EVERY_N_CANDLES = Math.max(1, Math.floor(marketData.length / 40)); // حدود ۴۰ گزارش پیشرفت در کل بکتست
            ...
            for (let i = 20; i < marketData.length; i++) {
                ...
                // ==================== لاگ پیشرفت دوره‌ای ====================
                if (i % LOG_EVERY_N_CANDLES === 0) {
                    console.log(`⏱️ [PROGRESS] کندل ${i}/${marketData.length} | زمان: ${candle.timestamp} | close: ${candle.close} | پوزیشن‌های باز: ${positions.length} | معاملات تاکنون: ${trades.length} | سرمایه: ${capital.toFixed(2)}`);
                }
                ...
                if (i % LOG_EVERY_N_CANDLES === 0) {
                    const pivotCount = (trendLineIncState && trendLineIncState.pivots) ? trendLineIncState.pivots.length : 0;
                    const byType = {};
                    processedTrendLines.forEach(l => { byType[l.type] = (byType[l.type] || 0) + 1; });
                    console.log(`   📏 [TRENDLINES] کندل ${i}: خطوط معتبر فعلی=${processedTrendLines.length} | پیوت‌های شناسایی‌شده=${pivotCount} | تفکیک نوع=${JSON.stringify(byType)}`);
                    if (ichimoku) {
                        console.log(`   ☁️ [ICHIMOKU] tenkan=${ichimoku.tenkan?.toFixed?.(4)} kijun=${ichimoku.kijun?.toFixed?.(4)} kumoTop=${ichimoku.kumoTop?.toFixed?.(4) ?? 'null'} kumoBottom=${ichimoku.kumoBottom?.toFixed?.(4) ?? 'null'} priceAboveCloud=${ichimoku.isPriceAboveCloud} priceBelowCloud=${ichimoku.isPriceBelowCloud} tenkanAboveKijun=${ichimoku.isTenkanAboveKijun} chikouBullish=${ichimoku.isChikouBullish}`);
                    } else {
                        console.log(`   ☁️ [ICHIMOKU] null (داده تاریخی کافی نیست یا options.ichimoku.enabled=false)`);
                    }
                }
                ...
                if (processedTrendLines.length > 0) {
                    processedTrendLines.forEach(line => {
                        ...
                        if (breakInfo) {
                            ...
                            console.log(`   💥 [BREAK] کندل ${i}: خط ${line.id} (${line.type}) شکسته شد | جهت=${breakInfo.direction} | قیمت شکست=${breakInfo.breakPrice} | مقدار خط در این کندل=${breakInfo.lineValueAtBreak.toFixed(4)} → سیگنال برای کندل ${i + 1} ثبت شد`);
                ...
                try {
                    ...
                    if (allowNewTrade && signal && signal.signal) {
                        ...
                        if (capital < requiredCapital) {
                            ...
                            console.log(`⚠️ [CAPITAL_BLOCK] سرمایه کافی نیست در کندل ${i}: سرمایه فعلی=${capital.toFixed(2)} < نیاز=${requiredCapital.toFixed(2)}`);
                        }
            ...
            console.log(`└─ پوزیشن‌های واقعاً بازشده: ${debugStats.positionsOpenedCount} (اولین در کندل ${debugStats.firstPositionCandleIndex ?? 'هیچ‌وقت'})`);
