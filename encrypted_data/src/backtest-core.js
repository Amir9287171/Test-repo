...
async function runBacktest(marketData, options, onProgress) {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            // وقتی inverseMode فعال است:

            //   ۱. باز کردن پوزیشن جدید فقط در کندل‌هایی مجاز است که timestamp آن‌ها دقیقاً

            //      برابر یکی از زمان‌های ورود استخراج‌شده از معاملات نسخه‌ی اصلی

            //      (options.inverseEntryTimes) باشد؛ نه بر اساس entryStartIndex معمولی.

            //   ۲. محدودیت ۰.۵٪ (canOpenNewPosition) کاملاً غیرفعال می‌شود، چون زمان‌های

            //      ورود از قبل مشخص شده‌اند و ممکن است فاصله‌ی آن‌ها از هم کمتر از ۰.۵٪ باشد.

            // تابع customStrategy همچنان (مثل حالت عادی) در هر کندل فراخوانی می‌شود؛ فقط

            // اجازه‌ی *باز کردن* پوزیشن به کندل‌های منطبق با زمان‌های ورود محدود می‌شود.

            const inverseMode = options.inverseMode === true;

            const inverseEntryTimesSet = inverseMode

                ? new Set((options.inverseEntryTimes || []).map(t => {
            ...
            if (inverseMode) {
                ...
                if (!inverseEntryTimesSet || inverseEntryTimesSet.size === 0) {

                    throw new Error('حالت بکتست معکوس (inverseMode) فعال است اما options.inverseEntryTimes خالی یا نامعتبر است.');

                }

                console.log(`🔁 [INVERSE] حالت بکتست معکوس فعال است: ${inverseEntryTimesSet.size} زمان ورود منحصربه‌فرد از نسخه‌ی اصلی بارگذاری شد. محدودیت ۰.۵٪ (canOpenNewPosition) غیرفعال است.`);
            ...
            // اضافه شد: دقیقاً مثل trendLineSettings/ichimokuSettings، حالا می‌شود از

            // options.divergenceSettings.config یک آبجکت جزئی (فقط کلیدهایی که می‌خواهید

            // عوض شود، مثل PIVOT_PERIOD یا RSI_PERIOD) پاس داد تا برای همین بک‌تست، روی

            // تنظیمات پیش‌فرض DIVERGENCE_CONFIG داخل divergence-detector.js اعمال شود —

            // بدون نیاز به ویرایش آن فایل. اگر داده نشود، رفتار دقیقاً مثل قبل (پیش‌فرض‌ها).

            const divergenceConfigOverride = (divergenceSettings && divergenceSettings.config && typeof divergenceSettings.config === 'object')

                ? divergenceSettings.config
            ...
            if (divergenceConfigOverride) {

                console.log('⚙️ [DIVERGENCE] config سفارشی برای این بک‌تست:', JSON.stringify(divergenceConfigOverride));
            ...
            // ==================== پیش‌محاسبه‌ی batch اندیکاتورهای واگرایی (اصلاح OOM) ====================

            // قبلاً اینجا نوشته شده بود که «runDivergenceDetection هر بار RSI/MACD را از صفر روی

            // کل marketData محاسبه می‌کند» و فقط با recomputeInterval throttle می‌شد — یعنی

            // مشکل اصلی (O(n) کار برای هر بازمحاسبه) حل نشده بود، فقط تعداد بازمحاسبه‌ها کم

            // می‌شد. این باعث می‌شد روی بک‌تست‌های طولانی/چندفایلی (خصوصاً وقتی خیلی فایل و

            // تایم‌فریم پشت‌سرهم در یک پروسه‌ی node اجرا می‌شوند)، مصرف حافظه به‌مرور آن‌قدر بالا

            // برود که با خطای «JavaScript heap out of memory» کرش کند — دقیقاً چیزی که در

            // نسخه‌ی قبلی سیستم رخ می‌داد و فقط استراتژی‌های واگرایی را درگیر می‌کرد (چون خطوط

            // روند/ایچیموکو از قبل با precomputeTrendLinesBatch یک‌بار محاسبه می‌شدند).

            //

            // RSI و MACD کاملاً causal هستند (مقدار هر ایندکس فقط از کندل‌های تا همان ایندکس

            // می‌آید)، پس محاسبه‌ی یک‌باره‌ی کل آرایه پیش از حلقه و خواندن مقدار هر ایندکس در

            // ادامه، خروجی را عیناً حفظ می‌کند — فقط کار تکراری O(n²) را به O(n) کاهش می‌دهد.

            const divergencePrecomputed = {};

            if (divergenceDetector && divergenceIndicators.length > 0) {

                const precomputeEffectiveConfig = divergenceDetector.mergeDivergenceConfig

                    ? divergenceDetector.mergeDivergenceConfig(divergenceConfigOverride)

                    : (divergenceConfigOverride ? Object.assign({}, divergenceDetector.DIVERGENCE_CONFIG, divergenceConfigOverride) : divergenceDetector.DIVERGENCE_CONFIG);

                console.log('⚙️  [BATCH] پیش‌محاسبه‌ی اندیکاتورهای واگرایی (RSI/MACD) به‌صورت batch روی کل دیتاست...');

                const __divBatchT0 = Date.now();

                for (const indName of divergenceIndicators) {

                    if (indName === 'RSI') {

                        divergencePrecomputed.RSI = divergenceDetector.precomputeIndicator(marketData, 'RSI', precomputeEffectiveConfig);

                    } else if (indName === 'MACD') {

                        divergencePrecomputed.MACD = divergenceDetector.precomputeIndicator(marketData, 'MACD', precomputeEffectiveConfig);

                    }

                }

                console.log(`⚙️  [BATCH] اندیکاتورهای واگرایی (${divergenceIndicators.join(', ')}) در ${Date.now() - __divBatchT0}ms پیش‌محاسبه شدند.`);
            ...
            const settings = {
                ...
                // (بافر ثابت FIXED_BUFFER_CANDLES = 720 کندل).
            ...
            // ==================== بافر ثابت بر اساس تعداد کندل (بدون پارامتر جدید در استراتژی) ====================

            // FIXED_BUFFER_CANDLES: تعداد کندل بافر که برای همه‌ی تایم‌فریم‌ها (5m/15m/30m/1h/4h/24h)
            // به‌صورت یکسان و ثابت اعمال می‌شود. قبلاً این مقدار به‌صورت درصدی از طول فایل قبلی
            // محاسبه می‌شد (15m→75%, 30m→125%, 1h→200%) که با اضافه شدن تایم‌فریم‌های خیلی
            // بزرگ‌تر (4h, 24h) دیگر منطقی نبود. الان صرفاً «۷۲۰ کندل آخرِ همان تایم‌فریم» به
            // عنوان بافر استفاده می‌شود، مستقل از طول فایل یا برچسب تایم‌فریم.
            ...
            // calculateBufferSize: دقیقاً (حداکثر) targetCount کندل آخر را از یک یا چند فایل/تکه‌ی
            // قبلی می‌سازد. کاملاً مستقل از تایم‌فریم یا طول فایل است.
            //   prevDataList: آرایه‌ای از داده‌های قبلی، از جدیدترین (prevDataList[0]) به قدیمی‌ترین
            //                 (یا یک آرایه‌ی صاف از کندل که خودش قبلاً حداکثر ۷۲۰ تاست).
            //   targetCount: تعداد کندلی که باید در بافر باشد (پیش‌فرض FIXED_BUFFER_CANDLES=720).
            // اگر مجموع کندل‌های موجود کمتر از targetCount باشد، هر چه در دسترس است برگردانده
            // می‌شود (بدون خطا) — بافر فقط از داده‌های گذشته ساخته می‌شود، پس هیچ آینده‌نگری‌ای
            // رخ نمی‌دهد.
            function calculateBufferSize(prevDataList, targetCount) {

                const count = targetCount || FIXED_BUFFER_CANDLES;
                ...
                for (let fileIdx = 0; fileIdx < normalized.length && remaining > 0; fileIdx++) {

                    const file = normalized[fileIdx];

                    if (!file || file.length === 0) continue;

                    if (file.length <= remaining) {

                        bufferParts.unshift(file); // کل این تکه/فایل داخل بافر جا می‌شود

                        remaining -= file.length;

                    } else {

                        bufferParts.unshift(file.slice(file.length - remaining)); // فقط دنباله‌ی لازم
            ...
            // (FIXED_BUFFER_CANDLES = 720 کندل ثابت) به‌صورت خودکار به ابتدای marketData الحاق می‌شود تا
            ...
            if (settings.enableSmartContinuation) {
                ...
                if (prevList.length > 0) {
                    ...
                    if (entryStartIndex > 0) {
                        ...
                        console.log(`🧩 بافر خودکار ابتدای فایل: ${entryStartIndex} کندل (از حداکثر ${FIXED_BUFFER_CANDLES} کندل ثابت، تایم‌فریم ${settings.timeframeLabel || 'نامشخص'}) الحاق شد؛ معاملات جدید فقط از کندل ${entryStartIndex} به بعد مجازند`);
            ...
            // اگر options.stopLossStages مشخص شده باشد (مثلاً از ANALYSIS_CONFIG فایل استراتژی،

            // شامل حالت _INVERSE.js که باید حد ضرر پلکانی خودش را تعیین کند)، به‌جای مقدار

            // پیش‌فرض هاردکد از همان استفاده می‌شود. این مقدار به‌عنوان fallback برای موقعیت‌هایی

            // است که signal.stopLossStages را صریحاً برنمی‌گردانند.

            const stopLossStages = Array.isArray(options.stopLossStages) && options.stopLossStages.length > 0

                ? options.stopLossStages
            ...
            function findNextFileByDate(referenceData, usedFileNames = new Set()) {
                ...
                // اندازه‌ی بافر خودکار — فقط جهت اطلاع؛ محاسبه‌ی نهایی و استخراج واقعی بافر

                // (که می‌تواند از چند فایل قبلی تشکیل شود) در buildCombinedData انجام می‌شود.

                // ثابت برای همه‌ی تایم‌فریم‌ها: حداکثر FIXED_BUFFER_CANDLES (۷۲۰) کندل.

                const calculatedBufferSize = Math.min(dataToCheck.length, FIXED_BUFFER_CANDLES);
            ...
            // ==================== تابع ساخت داده‌ی ترکیبی (بافر ثابت ۷۲۰ کندل + فایل بعدی) ====================

            // بدون نیاز به هیچ پارامتر جدیدی در استراتژی: صرف‌نظر از timeframeLabel، همیشه حداکثر

            // FIXED_BUFFER_CANDLES (۷۲۰) کندل آخر از یک یا چند فایل/تکه‌ی قبلی (prevDataList، از
            ...
            // رخ نمی‌دهد. timeframeLabel فقط برای لاگ نگه داشته شده است.
            ...
            function buildCombinedData(prevDataList, nextData, timeframeLabel) {
                ...
                const bufferData = calculateBufferSize(normalizedPrevList, FIXED_BUFFER_CANDLES);
                    ...
                    for (let j = remainingPositions.length - 1; j >= 0; j--) {
                        ...
                        if (!shouldExit && position.useStagedStopLoss) {
                            ...
                            const newStopLoss = stagedStopLoss(
                                ...
                                position.type === 'BUY' ? candle.high : candle.low,
            ...
            for (let i = 20; i < marketData.length; i++) {
                    ...
                    for (const indicatorName of divergenceIndicators) {
                        ...
                        try {
                            ...
                            const sigs = divergenceDetector.runDivergenceDetection({
                                ...
                                config: divergenceConfigOverride,

                                precomputedIndicatorValues: indicatorName === 'RSI' ? divergencePrecomputed.RSI : undefined,

                                precomputedIndicatorData: indicatorName === 'MACD' ? divergencePrecomputed.MACD : undefined
                ...
                let allowNewTrade = inverseMode

                    // حالت معکوس: باز کردن پوزیشن فقط در کندل‌هایی مجاز است که timestamp آن‌ها

                    // دقیقاً منطبق بر یکی از زمان‌های ورود نسخه‌ی اصلی باشد (entryStartIndex/بافر

                    // خودکار ادامه‌دهی در این حالت بی‌اثر است).

                    ? inverseEntryTimesSet.has(new Date(candle.timestamp).getTime())
                ...
                try {
                    ...
                    // در حالت بکتست معکوس، محدودیت ۰.۵٪ (canOpenNewPosition) کاملاً غیرفعال است؛

                    // چون زمان‌های ورود از قبل و از نسخه‌ی اصلی مشخص شده‌اند و ممکن است فاصله‌ی

                    // آن‌ها از هم کمتر از ۰.۵٪ باشد.

                    if (!inverseMode && allowNewTrade && signal && signal.signal) {
            ...
            if (positions.length > 0 && settings.enableContinuation) {
                ...
                // تاریخچه‌ی فایل‌های قبلی (از جدیدترین به قدیمی‌ترین)، برای پشتیبانی از بافر ثابت

                // FIXED_BUFFER_CANDLES (۷۲۰ کندل) که ممکن است به بیش از یک فایل قبلی نیاز داشته باشد.

                let fileHistory = [marketData];

                const MAX_FILE_HISTORY = 5; // مقدار محافظه‌کارانه برای پوشش ۷۲۰ کندل حتی وقتی فایل‌ها کوچک‌اند
                ...
                while (remainingPositions.length > 0 && continuationCount < settings.maxContinuationFiles) {
                    ...
                    // بعدی، بافر ثابت (FIXED_BUFFER_CANDLES = 720 کندل) از انتهای فایل(های) قبلی

                    // را به‌صورت خودکار به ابتدای آن الحاق می‌کنیم تا اندیکاتورها در ابتدای فایل
                    ...
                    if (settings.enableSmartContinuation) {
                        ...
                        if (combined.bufferSize > 0) {
                            ...
                            console.log(`🧩 بافر خودکار ادامه‌دهی: ${combined.bufferSize} کندل (از حداکثر ${FIXED_BUFFER_CANDLES} کندل ثابت، تایم‌فریم ${settings.timeframeLabel || 'نامشخص'}) به ابتدای فایل بعدی الحاق شد`);
            ...
            if (settings.enableSmartContinuation && smartContinuationStats.usedCount > 0) {
                ...
                console.log(`├─ اندازه‌ی بافر خودکار: ${smartContinuationStats.lastBufferSize} کندل (از حداکثر ${FIXED_BUFFER_CANDLES} کندل ثابت، تایم‌فریم ${settings.timeframeLabel || 'نامشخص'})`);
