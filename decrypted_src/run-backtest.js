...
const chunkSize  = parseInt(getArg('--chunk-size') || '5');
const dataDir    = getArg('--data-dir') || path.join(process.cwd(), 'data');

log(`شروع run-backtest.js با آرگومان‌ها: strategyFile=${strategyFile}, startIndex=${startIndex}, chunkSize=${chunkSize}, dataDir=${dataDir}`);
...
(async () => {
    ...
    // ذخیره strategyCode در 1.json یک بار قبل از حلقه
    ...
    for (let fileIdx = 0; fileIdx < fileNames.length; fileIdx++) {
        ...
        try {
            ...
            log(`✅ results.enc برای ${fileName} ذخیره شد.`);
